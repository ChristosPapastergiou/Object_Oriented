...
#define MinTolls 3     //Minimun and maximum amount of tolls
#define MaxTolls 4     //in ever entrance
#define MinSegCap 10   //Minimun and maximum 
#define MaxSegCap 20   //segment capacity of vehicles
...
//Attiki's C/D
Attiki::Attiki(int k, int nsegs):NSegs(nsegs), Total_Vehicles(0){ //Initialize
    cout << "Highway in use." << endl;
    this->segment = new Segment*[NSegs]; //Allocate memory for n segments
    for(int i=0; i<this->NSegs; i++){
        this->segment[i] = new Segment(k, NSegs, i+1); //Calling constructor to make n segments
    }
    for(int i=0; i<this->NSegs; i++){
        if(i==0){
...
Attiki::~Attiki(){
    for (int i=0; i<this->NSegs; i++){  //Freeing the allocs i did for the n segments
        delete segment[i];              
    }
    delete[] segment;
}
...
//Toll's Cs/Ds (toll parent class)
